...
class GraphLayerAtt(torch.nn.Module):
    ...
    ----------
    dims_in: tuple
        2-element tuple containing input dimension for nodes, and edge,
        respectively.
    dims_out: tuple
        2-element tuple containing output dimension for nodes, and edge,
        respectively.
    weight_init_func: function, optional
        Weight initialization function.
    ...
    def __init__(self, dims_in, dims_out, weight_init_func=None):
        ...
        in_v, in_e = dims_in
        out_v, out_e = dims_out

        self.dense_vv = torch.nn.Linear(in_v, out_v)
        self.dense_ve = torch.nn.Linear(in_e, out_v, bias=False)
        self.attention_ve = torch.nn.Linear(in_e, 1)

        self.dense_ee = torch.nn.Linear(in_e, out_e)
        self.dense_ev = torch.nn.Linear(in_v, out_e, bias=False)

        if weight_init_func is not None:
            weight_init_func(self.dense_vv.weight)
            weight_init_func(self.dense_ve.weight)
            weight_init_func(self.dense_ee.weight)
            weight_init_func(self.dense_ev.weight)


    def forward(self, x_v, x_e, arc_index):
        ...
        e = self.attention_ve(x_e).squeeze(-1)   # (m,)
        ...
        msg_from_arcs = self.dense_va(x_e) * attn
        msg_from_arcs_src = self.dense_va(x_e)* attn_src
